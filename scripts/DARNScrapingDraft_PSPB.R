...
download_files = function(volume) {
  ...
  if (volume < 6){
    ...
    pspb_decade=paste('d', toString((volume + 1974) - ((volume + 1974) %% 10)), sep='')
  } else {pspb_decade='ERROR'}
  pspb_year = paste('y',toString(volume+1974), sep='')
  
  volumeurl = paste(PSPB_VOLUME_BASE_URL, pspb_decade, sep = '') %>%
    paste(pspb_year, sep='.')
